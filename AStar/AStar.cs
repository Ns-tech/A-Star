...
namespace AStar
{
    class AStar
    {
        static void Main(string[] args)
        {
            ...
            DistanceType distanceType = DistanceType.m;

            FillGraphWithLatLng(graph);
            do
            {
                ...
                Console.WriteLine("A* Search - Sample");
        ...
        /// </summary>
        /// <param name="graph">The Graph to be filled</param>
        private static void FillGraphWithLatLng(Graph graph)
        {
            var data = new OpenExcel().ReadData();
            foreach (var item in data.points)
            {
                graph.AddNode(new Node(item.Id, null, item.Lat, item.Lng)); // Adding the Node to the Graph...
            }
            foreach (var item in data.directLinks)
            {
                graph.AddUndirectedEdge(item.CurrentPoint, item.NextPoint, item.Distance);
            }
        }
